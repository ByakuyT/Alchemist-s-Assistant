...
namespace AlchAssEx
{
    public static class Helper
    {
        ...
        {
            AlchAss.Helper.RegisterLoc("avortex", "Vortex control ", "漩涡制动");
            AlchAss.Helper.RegisterLoc("aclosest", "Closest control ", "最近点制动");
            AlchAss.Helper.RegisterLoc("aproximity", "Proximity control ", "接近点制动");
        }
        #endregion
...
ControlAsymptoteFactor=0.0015";
            File.WriteAllText(configFilePath, defaultConfig);
            Vars._functionCacheValid = true;
        }
        public static void UpdateFunctionsConfigCache()
        {
            try
            {
                var configFilePath = Path.Combine(AlchAss.Vars.ConfigDirectory, Vars.functionPath);
                ...
                {
                    { "SlowdownFactorX", value => Vars._cachedSlowdownFactorX = value },
                    { "SlowdownFactorZ", value => Vars._cachedSlowdownFactorZ = value },
                    { "BrewingMultiplierX", value => Vars._cachedBrewingMultiplierX = (int)value },
                    { "BrewingMultiplierZ", value => Vars._cachedBrewingMultiplierZ = (int)value },
                    { "HeatValue", value => Vars._cachedHeatValue = value },
                    { "GrindValue", value => Vars._cachedGrindValue = value },
                    { "ControlAreaThreshold", value => Vars._cachedControlAreaThreshold = value },
                    { "ControlSlowdownStrength", value => Vars._cachedControlSlowdownStrength = value },
                    { "ControlAsymptoteFactor", value => Vars._cachedControlAsymptoteFactor = value }
                };
