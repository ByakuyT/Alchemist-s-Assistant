...
namespace AlchAss
{
    public static class Functions
    {
        ...
        public static void VortexSelection()
        {
            ...
            bool hasSelection = Variables.selectedVortexIndex >= 0 && Variables.selectedVortexIndex < Variables.allVortexData.Count;
            if (hasSelection)
            ...
            if (nextPressed)
            {
                ...
                else if (Variables.selectedVortexIndex >= Variables.allVortexData.Count - 1)
                    Variables.selectedVortexIndex = -1;
                else
                    Variables.selectedVortexIndex++;
            }
            else
            {
                if (!hasSelection)
                    Variables.selectedVortexIndex = Variables.allVortexData.Count - 1;
                else if (Variables.selectedVortexIndex <= 0)
                    Variables.selectedVortexIndex = -1;
                else
                    Variables.selectedVortexIndex--;
            }
        ...
        public static void TargetCircleMode()
        {
            if (Keyboard.current.backspaceKey.wasPressedThisFrame)
            {
                ...
                if (Variables.directionLine && Variables.solventDirectionHint != null)
                    Traverse.Create(Variables.solventDirectionHint).Method("OnPositionOnMapChanged", Array.Empty<object>()).GetValue();
        ...
        public static void DirectionLine()
        {
            if (Keyboard.current.slashKey.wasPressedThisFrame)
            {
                ...
                {
                    Variables.savedEnablePathRendering = Variables.enablePathRendering;
                    Variables.savedUseAngleAdjustedRadius = Variables.useAngleAdjustedRadius;
                    if (Variables.enablePathRendering)
                    {
                        Depends.ShowOriginalPaths();
                        for (int i = 0; i < Variables.pathLineRenderer.Length; i++)
...
{LocalizationManager.GetText("direction_close")}{directionToClosest}'";
        ...
        public static void UpdateLineDirections()
        {
            ...
            if (Variables.SharedCache.isValid)
            {
                ...
                if (Variables.useAngleAdjustedRadius)
                {
                    ...
                    radiusValues = [1.53f, 1f / 3f - angleDelta / 216f, 1f / 18f - angleDelta / 216f];
                }
                else
                    radiusValues = [1.53f, 1f / 3f, 1f / 18f];
            ...
            if (pathHints.Count > 0)
            {
                ...
                if (firstHint.evenlySpacedPointsFixedPhysics?.points != null && firstHint.evenlySpacedPointsFixedPhysics.points.Length >= 2)
                {
                    bool isTeleportation = firstHint.GetType().Name == "TeleportationFixedHint";
                    ...
                    Variables.lineDirection[2] = Vector2.SignedAngle(Vector2.right, endPosition - startPosition);
                    Variables.lineRenderer[2].enabled = true;
                }
                else
                    Variables.lineRenderer[2].enabled = false;
            }
            ...
            if(indicatorPosition.sqrMagnitude > 0)
            {
                ...
                Variables.lineRenderer[1].enabled = false;
            foreach (var vortex in targetVortexes)
            {
            ...
            if (Variables.SharedCache.isValid)
            {
                Variables.lineDirection[0] = Vector2.SignedAngle(Vector2.right, Variables.SharedCache.targetPosition - indicatorPosition);
                Variables.lineRenderer[0].enabled = true;
            }
            else
                Variables.lineRenderer[0].enabled = false;
            if (Variables.selectedVortexIndex >= 0 && Variables.selectedVortexIndex < Variables.allVortexData.Count)
            {
                var selectedVortex = Variables.allVortexData[Variables.selectedVortexIndex];
                ...
                Variables.lineRenderer[3].enabled = true;
            }
            else
                Variables.lineRenderer[3].enabled = false;
        }
        public static void CollectPathPointsForCalculation(IList<FixedHint> pathHints)
        {
            Variables.normalPathSegments.Clear();
            List<Vector3> currentNormalSegment = [];
            ...
            foreach (var hint in pathHints)
            {
                ...
                if (hint.evenlySpacedPointsFixedPhysics?.points != null)
                {
                    if (isTeleportation)
                    {
                        ...
                        {
                            Variables.normalPathSegments.Add([.. currentNormalSegment]);
        ...
        public static void CollectPathsByIngredient(IList<FixedHint> pathHints)
        {
            ...
            for (int i = 0; i < pathHints.Count; i++)
            {
                ...
                bool isTeleportation = hint.GetType().Name == "TeleportationFixedHint";
                if (hint.evenlySpacedPointsFixedGraphics?.points == null || hint.evenlySpacedPointsFixedGraphics.points.Length <= 1)
                    continue;
