...
[![版本](https://img.shields.io/badge/版本-4.1.0-blue.svg)](https://github.com/user/AlchAss)
[![游戏名](https://img.shields.io/badge/游戏名-Potion%20Craft-green.svg)](https://store.steampowered.com/app/1210320/Potion_Craft_Alchemist_Simulator/)
...
#### 🌪️ 漩涡信息窗口
提供实时的漩涡几何数据分析，支持智能漩涡检测：
- **智能漩涡检测**：优先计算当前接触的漩涡，不在漩涡内时计算选定的漩涡
- **当前距离**：药水瓶到漩涡中心的精确距离
- **最大距离**：可以安全接触漩涡的最大距离
- **当前夹角**：加水方向与漩涡方向的夹角
- **最大夹角**：可以进行外拉操作的最大角度限制
- **安全区域计算**：基于漩涡半径和药水瓶半径的精确计算
- **漩涡选择**：通过逗号(,)和句号(.)键切换选择不同漩涡
...
**漩涡交会点标记**：
- **智能计算**：实时计算并显示当前搅拌和加水路径与目标漩涡的交会点
- **重置方式**：标记会持续显示直到不存在目标漩涡
...
| 按键 | 功能 | 详细说明 |
|------|------|----------|
| `/` | 辅助示线开关 | 显示/隐藏搅拌、加水、目标方向线、位置标记和漩涡范围线 |
...
| 按键 | 功能 | 详细说明 |
|------|------|----------|
| `'` | 漩涡制动开关 | 开启后接近漩涡边缘时自动减速到停止 |
| `]` | 最近点制动开关 | 开启后接近目标最近点时自动减速到停止 |
| `[` | 接近点制动开关 | 开启后最近点接近目标效果位置时自动减速相关操作 |
| `,` | 上一个漩涡 | 切换到距离当前位置更近的上一个漩涡 |
| `.` | 下一个漩涡 | 切换到距离当前位置更远的下一个漩涡 |
...
- `AlchAssVortexData_XX.txt` - 漩涡位置记录（分地图扫描后各自保存）
